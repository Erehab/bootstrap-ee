  ...
  $('#nl-signup').on('submit', function(event) { 
    ...
    var name =null;
      // if(nl.name.length>0){
        name = $('#nl-name').val()
      // }
    $.ajax({url: 'https://ptclinic.com/site/signup_processorKB.php',
      type: 'GET',
      data: {
        'practice_id':nl.pid,
        'contact_name':name,
        ...
        if(s['success']=='true'){
          ...
          $('#nl-signup').empty().css({'padding':'0px 2px','textAlign':'center','color':'#E74C3C'}).html('<span>Invalid Submittion.</span>');
        }
      }
    });
